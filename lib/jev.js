...
export function buildJevRequest(snapshot) {
  ...
  return {
    ...
    state: {
      ...
      objective: `Choose the legal move that gives ${currentPlayer} the best chance of winning. A draw is preferable to a loss.`,
    },
    questions: {
      best_move: choice(`Which legal move should ${currentPlayer} make now?`, criteria),
    },
  };
}
